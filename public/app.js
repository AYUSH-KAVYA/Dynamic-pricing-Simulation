/* ======================================================
   ...
   Cyclical Inventory + Profit-Constrained Economic Engine
   ====================================================== */
...
function toggleSidebar() {
    ...
    // Update the ◀ / ▶ arrow inside the sidebar header
    toggle.textContent = isCollapsed ? '▶' : '◀';

    // Show the floating re-open button when sidebar is hidden
    expand.style.display = isCollapsed ? 'flex' : 'none';

    // Animate the grid so main content fills the freed space
    dashboard.style.gridTemplateColumns = isCollapsed ? '0 1fr' : '320px 1fr';
}
...
async function autoTrain() {
    ...
    try {
        ...
        // Auto-fill form with learned parameters (inventory policy keeps defaults)
...
function getFormValues() {
    return {
        ...
        targetMargin: Number(document.getElementById('targetMargin').value) / 100,
        ...
        restockQuantity: Number(document.getElementById('restockQuantity').value),
...
// ── Profitable Default Inputs ──────────────────────────────────────────────
// All fields are unique, all non-zero, calibrated for StableProfit strategy.
function resetForm() {
    document.getElementById('initialPrice').value = 1400;
    document.getElementById('unitCost').value = 1184;
    document.getElementById('targetMargin').value = 20;
    document.getElementById('initialInventory').value = 400000;
    document.getElementById('baseDemand').value = 95000;
    document.getElementById('alpha').value = 15;
    document.getElementById('referencePrice').value = 1350;
    document.getElementById('competitorPrice').value = 1330;
    document.getElementById('gamma').value = 0.25;
    document.getElementById('beta').value = 1.2;
    document.getElementById('reorderPoint').value = 100000;
    document.getElementById('restockQuantity').value = 250000;
    document.getElementById('leadTime').value = 3;
    document.getElementById('simulationDays').value = 30;
    document.getElementById('strategy').value = 'StableProfit';
    document.getElementById('competitorModel').value = 'reactive';
    document.getElementById('adjustmentRate').value = 3;

    // Reset KPI display
    ['kpiRevenueValue', 'kpiProfitValue', 'kpiUnitsValue', 'kpiInventoryValue',
        'kpiMarketShareValue', 'kpiLostSalesValue', 'kpiLostRevenueValue']
...
async function runSimulation() {
    ...
    try {
        ...
        animateValue('kpiLostSalesValue', kpis.totalLostSales);
        animateValue('kpiLostRevenueValue', kpis.totalLostRevenue, '₹');
        ...
        const floorNote = kpis.priceFloor ? ` | Price floor: ₹${kpis.priceFloor.toLocaleString('en-IN')}` : '';
        showStatus(`✅ Simulation complete — ${timeSeries.length} days simulated${floorNote}`, 'success');
    } catch (err) {
...
function updateAnalysisTable(timeSeries) {
    ...
    tbody.innerHTML = timeSeries.map(d => {
        ...
        const lostClass = d.lostSales > 0 ? 'negative' : '';
        return `<tr>
      <td>${d.day}</td>
      ...
      <td class="${lostClass}">${d.lostSales.toLocaleString()}</td>
      <td>₹${d.revenue.toLocaleString()}</td>
      <td class="${profitClass}">₹${d.profit.toLocaleString()}</td>
      <td class="${lostClass}">₹${d.lostRevenue.toLocaleString()}</td>
...
function exportCSV() {
    ...
    const headers = ['Day', 'Price', 'Competitor Price', 'Demand', 'Sales', 'Lost Sales', 'Revenue', 'Profit', 'Lost Revenue', 'Inventory', 'Market Share', 'Elasticity'];
    const rows = lastTimeSeries.map(d => [
        d.day, d.price, d.competitorPrice, d.demand, d.sales, d.lostSales,
        d.revenue, d.profit, d.lostRevenue, d.inventory,
        (d.marketShare * 100).toFixed(1), d.elasticity?.toFixed(2) || ''
    ]);
...
function getChartColors() {
    ...
    return {
        ...
        demand: { line: '#a78bfa', fill: 'rgba(167, 139, 250, 0.1)' },
        sales: { line: '#34d399', fill: 'rgba(52, 211, 153, 0.15)' },
    };
}
...
function createPriceChart() {
    ...
    return new Chart(ctx, {
        ...
        options: {
            responsive: true, maintainAspectRatio: true,
            interaction: { intersect: false, mode: 'index' },
            plugins: { legend: { display: true, labels: { color: getChartColors().text, usePointStyle: true, padding: 16 } }, tooltip: { backgroundColor: 'rgba(15,17,23,0.9)', titleColor: '#f1f5f9', bodyColor: '#94a3b8', borderColor: 'rgba(99,102,241,0.3)', borderWidth: 1, padding: 10, cornerRadius: 8 } },
            scales: { x: { grid: { color: getChartColors().grid, drawBorder: false }, ticks: { color: getChartColors().text, maxTicksLimit: 10 } }, y: { grid: { color: getChartColors().grid, drawBorder: false }, ticks: { color: getChartColors().text, callback: (v) => '₹' + v.toLocaleString() } } },
        },
    });
}
...
async function loadHistory() {
    try {
        ...
        tbody.innerHTML = json.data.map(sim => {
            ...
            return `<tr>
        <td>${date}</td>
        <td>${strategyLabel(sim.strategy)}</td>
...
async function viewSimulation(id) {
    ...
    try {
        ...
        animateValue('kpiLostSalesValue', sim.totalLostSales || 0);
        animateValue('kpiLostRevenueValue', sim.totalLostRevenue || 0, '₹');
...
async function deleteSimulation(id) {
    try {
        await fetch(`${API_BASE}/api/simulate/${id}`, { method: 'DELETE' });
        loadHistory();
    } catch (err) {
